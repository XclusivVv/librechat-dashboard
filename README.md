...
cd ~/bin  # or create: mkdir -p ~/bin && cd ~/bin
...
Add this content (replace `YOUR_MASTER_KEY_HERE` with your generated key):
```ini
[Unit]
...
ExecStart=/home/x/bin/meilisearch --http-addr 127.0.0.1:7700 --env production --master-key YOUR_MASTER_KEY_HERE
Restart=on-failure
...
### RAG API
```bash
cd ~/rag_api                      # Navigate to RAG API directory
...
### LibreChat
```bash
cd ~/LibreChat                    # Navigate to directory
...
- **Storage locations:**
  ...
  - RAG API: `~/rag_api` (~1.4GB with lite requirements)
- **Features:**
